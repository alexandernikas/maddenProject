...
def wideReciever():
    ...
    data = requests.get('https://ratings-api.ea.com/v2/entities/m23-ratings?filter=iteration:launch-ratings%20AND%20position:(WR)&sort=overall_rating:DESC,firstName:ASC&limit=50&offset=0').json()
    df = pd.DataFrame()
    tl.clean(data, df)
